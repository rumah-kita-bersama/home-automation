...
def get_prayer_time() -> dict:
    response = requests.get(
        'https://api.pray.zone/v2/times/today.json', {"city": "singapore", "school": 10}).json()

    if response['status'] == 'OK':
        prayer_times = response['results']['datetime'][0]['times']
        return {prayer: time for prayer, time in prayer_times.items() if prayer in PRAYER_WHITELIST}
    return dict()
...
def get_muis_prayer_time() -> dict:
    ...
    for prayer in MUIS_PRAYER_LIST:
        element = soup.find(
            'span', {'id': f'PrayerTimeControl1_{prayer["name"]}'})
